...
function App() {
  ...
  const handleWantToCook = (id) => {
    const itemFromWantToCook = wantToCooks.find(item => item.recipe_id === id);
    if(itemFromWantToCook) {
      Swal.fire({
        icon: "error",
        title: "Oops...",
        text: "Already Added to Want to Cook!",
      });
    }
    else {
        ...
        <div className="flex flex-col-reverse lg:flex-row gap-10">
          ...
          <div className="lg:w-[40%] my-8 p-2 border rounded-xl">
            <div className="want-to-cook ">
              <h1 className=" border-b-2 text-2xl font-bold text-center ">Want to cook: {wantToCooks.length}</h1>
             ...
             <div className="bg-[#28282808]">
              {
                ...
                wantToCooks.map((wantToCook, idx)=> <WantToCook key={idx} wantToCook={wantToCook} handleCurrentlyCooking={handleCurrentlyCooking}></WantToCook>)
              }
            ...
            <h1 className=" border-b-2 text-2xl font-bold text-center  ">Currently Cooking: {currentlyCookings.length}</h1>
             <div className="flex ml-5  gap-8 mt-3">
             <p>Name</p>
              <p>Time</p>
              <p>Calories</p>
             ...
             <div className="bg-[#28282808] ">
              {
                currentlyCookings.map((currentlyCooking, idx) => <CurrentlyCooking key={idx} currentlyCooking={currentlyCooking}></CurrentlyCooking>)
              }
             </div>
            </div>
          </div>
